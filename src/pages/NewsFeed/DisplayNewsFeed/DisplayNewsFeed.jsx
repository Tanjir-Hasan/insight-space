...
const DisplayNewsFeed = ({ query }) => {
  ...
  const [, handleBookMark, , , , handleDeletePost, handleUpdatePost] =
  ...
  const handleCopyClick = (id) => {
    ...
    tempInput.value = `${currentURL}/posts/${id}`;
    tempInput.select();
                ...
                <div className="flex justify-between">
                  <div className="flex gap-2 space-x-2 mb-4">
                    <div className="">
                      ...
                      <div className="absolute -bottom-1 -right-2">
                        {bages?.email === p.userEmail &&
                        bages.memberShip === "Basic" ? (
                          <img
                            className="w-7 h-7 rounded-full"
                            src="https://i.ibb.co/r0BMFDp/verified-green-512.webp"
                            alt=""
                          />
                        ) : bages?.email === p.userEmail &&
                          ...
                          <img
                            ...
                            src="https://i.ibb.co/3dzNwLw/download-1-removebg-preview.png"
                            alt=""
                          />
                        ) : (
                          ""
                    ...
                    <div>
                      ...
                      <div className="flex space-x-2">
                        <h6 className="flex items-center text-xs">
                          ...
                          {moment(p.date).startOf("hour").fromNow()}
                        </h6>
                        <button>
                          {p.status === "Public" && (
                            <SlGlobe title={p.status}></SlGlobe>
                          )}
                          {p.status === "Friends" && (
                            <FaUserFriends title={p.status}></FaUserFriends>
                          )}
                          {p.status === "Only me" && (
                            <FaLock title={p.status}></FaLock>
                          )}
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="px-2 flex items-baseline">
                      ...
                      <div className="relative">
                        <ul className="p-4 space-y-2 w-72 bg-[#F0EFEB] text-black rounded-lg absolute top-0 right-0">
                          <li>
                            <button
                              hidden={
                                bookMarks.length === allPosts.length ||
                                p.userEmail === userDetails.email
                              }
                              className="posts-action-btn"
                              onClick={() =>
                                handleBookMark(p._id, userDetails?.email)
                              }
                            >
                              Bookmark
                            </button>
                          ...
                          <li>
                            <button
                              hidden={p.userEmail !== userDetails.email}
                              className="posts-action-btn"
                    ...
                    <button
                      ...
                      onClick={() => setIsOpenPostActions(p._id)}
                    >
                    ...
                    <span
                      ...
                      className={`${
                        theme === "dark"
                          ...
                          ? "text-[#3c6e71]"
                          : ""
                      } font-extrabold hover:underline hover:underline-offset-4 ms-2 text-sm text-[#3c6e71] cursor-pointer`}
                    >
                      See More
                    </span>
                    ...
                    <span
                      ...
                      className={`${
                        theme === "dark"
                          ...
                          ? "text-[#3c6e71]"
                          : ""
                      } font-extrabold hover:underline hover:underline-offset-4 ms-2 text-sm cursor-pointer`}
                    >
                      See Less
                    </span>
                  </span>
            ...
            <div className="p-4 text-black">
              <div className="flex justify-between">
                <div className="flex space-x-2 mb-4">
                  ...
                  <div>
                    ...
                    <h6 className="flex items-center text-xs">
                      <FaHistory className="me-2"></FaHistory>
                      {moment(editPost.date).startOf("hour").fromNow()}
                    </h6>
                  </div>
                </div>
                ...
                <div>
                  <button onClick={() => setIsOpenModal(!isOpenModal)}>
                    <SlClose className="text-3xl"></SlClose>
                  </button>
                </div>
              </div>
              ...
              <div className="m-2">
                <textarea
                  ...
                  name=""
                  id=""
                  className="w-full border-slate-300 border-2 rounded-xl p-2"
              ...
              <div className="w-full flex space-x-8 p-6">
                <button className="flex items-center">
                  <AiFillHeart
                    className={
                      editPost.react.includes(userDetails.email)
                        ? "text-2xl text-red-600 me-2"
                        : "text-2xl me-2"
                    }
                  ></AiFillHeart>{" "}
                  {editPost.react.length}
                </button>

                <button className="flex items-center">
                  <FaComment className="text-2xl me-2"></FaComment>{" "}
                  {editPost.comment.length}
                </button>
              </div>
              ...
              <div>
                <button
                  ...
                  className="w-full secondary-button-light"
                >
                  Update
                </button>
              </div>
