    ...
    return (
        <div className='animate-zoom-in '>
            <p className="lg:text-xl  pb-2 font-bold">
                Total Photos <span className='bg-[#344e41] text-white rounded-full lg:p-1 px-1 lg:px-3'>{myPost?.length}</span>
                ...
                {myPost?.map((image, index) => (
                    <div key={image?._id}>
                        <div onClick={openModal}>
                            <img onClick={() => handlePhoto(image?._id)} className='w-full h-20' src={image?.imgURL} alt="" />
                        </div>
                    </div>
                ))}
            </div>
